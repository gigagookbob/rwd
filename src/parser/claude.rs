...
/// Excludes sessions generated by Claude SDK automation (`entrypoint = "sdk-cli"`).
///
/// Temporary mitigation: `rwd today` currently aggregates all Claude logs under
/// `~/.claude/projects`, which can include many short, command-only E2E runs.
/// Filtering these sessions keeps today's summary focused on interactive work.
pub fn exclude_sdk_cli_sessions(entries: Vec<LogEntry>) -> Vec<LogEntry> {
    let mut excluded_session_ids: HashSet<String> = HashSet::new();
    for entry in &entries {
        match entry {
            LogEntry::User(e) if e.entrypoint.as_deref() == Some("sdk-cli") => {
                excluded_session_ids.insert(e.session_id.clone());
            }
            LogEntry::Assistant(e) if e.entrypoint.as_deref() == Some("sdk-cli") => {
                excluded_session_ids.insert(e.session_id.clone());
            }
            LogEntry::Progress(e) if e.entrypoint.as_deref() == Some("sdk-cli") => {
                excluded_session_ids.insert(e.session_id.clone());
            }
            _ => {}
        }
    ...
    entries
        ...
        .filter(|entry| match entry {
            LogEntry::User(e) => !excluded_session_ids.contains(&e.session_id),
            LogEntry::Assistant(e) => !excluded_session_ids.contains(&e.session_id),
            LogEntry::Progress(e) => !excluded_session_ids.contains(&e.session_id),
            LogEntry::System(e) => e
                .session_id
                .as_ref()
                .map(|id| !excluded_session_ids.contains(id))
                .unwrap_or(true),
            LogEntry::FileHistorySnapshot(_) | LogEntry::Other(_) => true,
        })
        .collect()
}
...
mod tests {
    ...
    #[test]
    fn test_exclude_sdk_cli_sessions_removes_full_session() {
        let entries = vec![
            serde_json::from_str::<LogEntry>(
        ...
        let filtered = exclude_sdk_cli_sessions(entries);
        let summaries = summarize_entries(&filtered);
    ...
    #[test]
    fn test_exclude_sdk_cli_sessions_keeps_non_sdk_sessions() {
